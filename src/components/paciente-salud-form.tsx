...
export function PacienteSaludForm({ pacienteId, salud }: { pacienteId: string; salud: PacienteSalud | null }) {
  ...
  const [medicacionAbierta, setMedicacionAbierta] = useState(false);
  const [asistenciaAbierta, setAsistenciaAbierta] = useState(false);
  const tieneMedicacion = Boolean(salud?.medicacion_toma);
  const tieneAsistencia = Boolean(salud && salud.asistencia_tipos.length > 0);
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2.5">
        {!medicacionAbierta ? (
          <button
            type="button"
            onClick={() => setMedicacionAbierta(true)}
            className="text-muted inline-flex w-fit items-center gap-1.5 rounded-full bg-black/5 px-3 py-1.5 text-xs font-medium hover:bg-black/10 dark:bg-white/10 dark:hover:bg-white/15"
          >
            <Plus size={12} /> {tieneMedicacion ? "Editar medicamento" : "Añadir medicamento"}
          </button>
        ) : (
          <>
      ...
      <div className="flex flex-col gap-2.5">
        {!asistenciaAbierta ? (
          <button
            ...
            className="text-muted inline-flex w-fit items-center gap-1.5 rounded-full bg-black/5 px-3 py-1.5 text-xs font-medium hover:bg-black/10 dark:bg-white/10 dark:hover:bg-white/15"
          >
            <Plus size={12} /> {tieneAsistencia ? "Editar complemento" : "Añadir complemento"}
          </button>
        ) : (
          <>
      ...
      {(medicacionAbierta || asistenciaAbierta) && (
        <button
          type="button"
          onClick={guardar}
          disabled={guardando}
